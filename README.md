...
- **Session Protection**: Strong session protection with signed cookies (SameSite, HTTPOnly)
...
- **Authentication**: Session-based (signed cookies via Starlette SessionMiddleware)
- **Encryption**: cryptography (Fernet)
- **Forms**: WTForms (standalone) with Jinja2 templates
...
3. **Configure Environment**
   ...
   - `SECRET_KEY`: Session secret (use a strong random value)
   - `ADMIN_USERNAME`: Your admin username
   - `ADMIN_PASSWORD`: Your admin password (must meet password requirements)
   - `DATABASE_URL`: SQLite database path (default: `sqlite:///instance/secure_portal.db`)
...
✓ Admin-only pages protected with dependencies
...
secure_portal/
├── app/
│   ├── __init__.py           # FastAPI app factory (lifespan, middleware)
│   ├── database.py           # SQLAlchemy setup (standalone)
│   ├── models.py             # Database models
│   ├── routes.py             # Route handlers (FastAPI APIRouter)
│   ├── services.py           # Business logic
│   ├── crypto.py             # Encryption/decryption
│   ├── decorators.py         # FastAPI dependencies (auth)
...
├── test_app.py               # Quick-start test suite
...
⚠️ **DEVELOPMENT ONLY**: This app uses Uvicorn's development mode. For production:

- Use a production ASGI server with proper process management
- Set `reload=False`
